...
int process_command(int number_of_arguments, char** arguments)
{
    ...
    for (int i = 0; i < NUM_OF_COMMANDS; i++)
    {
         ...
         {
            if(commands[i].num_of_args + 1 == number_of_arguments || commands[i].num_of_args == -1)
                return i;  //valid command with correct number of arguments
            else if(commands[i].num_of_args + 1 != number_of_arguments)
            {
                LIST_INSERT_HEAD(&foundCommands, &commands[i]);
                return CMD_INV_NUM_ARGS; //invalid number of arguments
            }
          }
    }


    //command not found but its chars maybe matched one or more chars
    for (int i = 0; i < NUM_OF_COMMANDS; i++)
    {
        char* temp = commands[i].name;
        j = 0;
        for(int x = 0; x < strlen(temp); x++)
        {

            if(arguments[0][j] == temp[x])
            {
                j++;
                if(strlen(arguments[0]) == j)
                {
                    if(!found){
                        LIST_INSERT_HEAD(&foundCommands, &commands[i]);
                        lastCommandIndex = i;
                        ...
                        //add command to the list --> "foundCommands"
                        LIST_INSERT_BEFORE(&foundCommands, &commands[lastCommandIndex],  &commands[i]);
                        lastCommandIndex = i;
                        break;
                    }
